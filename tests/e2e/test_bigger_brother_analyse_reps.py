...
class TestAnalyseReps:
    def test_happy_path(self, bb_instance, video_path, input_config):
        ...
        with mock.patch.object(bb_instance.pose_estimator, "process_video",
                               return_value=(frames, fps)):
    ...
    def test_too_few_frames_raises(self, bb_instance, video_path, input_config):
        ...
        with mock.patch.object(bb_instance.pose_estimator, "process_video",
                            return_value=(frames, 10.0)):
            with pytest.raises(ValueError, match="At least 20 frames"):
                bb_instance.analyse_reps(video_path, input_config)

    def test_visualise_flag_does_not_crash(self, bb_instance, video_path, input_config, monkeypatch):
