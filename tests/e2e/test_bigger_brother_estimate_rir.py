...
class TestEstimateRir:
    ...
    def test_happy_path(self, bb_instance, target_video, calib_video, input_config):
        ...
        with mock.patch.object(
            ...
            with mock.patch(
                ...
                with mock.patch(
                    "bigger_brother.compute_VL_curve", return_value=np.poly1d([0.1, 0.5, 10])
                ) as mock_vl:
                    with mock.patch(
                        "bigger_brother.estimate_rir_from_curve", return_value=dummy_rir
                    ) as mock_rir:
                        result = bb_instance.estimate_rir(
                            target_video, calib_video, input_config
                        )
    ...
    def test_insufficient_reps_raises(self, bb_instance, target_video, calib_video, input_config):
        ...
        with mock.patch.object(
            ...
            with mock.patch(
                "bigger_brother.compute_metrics", return_value=few_metrics
            ):
                # Let compute_VL_curve run for real – it will raise ValueError
                with pytest.raises(ValueError, match="has only 2 rep"):
                    bb_instance.estimate_rir(target_video, calib_video, input_config)

    def test_visualise_flag_passes_through(self, bb_instance, target_video, calib_video, input_config, monkeypatch):
        ...
        with mock.patch.object(
            ...
            with mock.patch(
                ...
                with mock.patch(
                    ...
                    with mock.patch(
                        "bigger_brother.estimate_rir_from_curve", return_value=0
                    ):
                        bb_instance.estimate_rir(target_video, calib_video, input_config)
