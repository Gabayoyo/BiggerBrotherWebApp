...
def create_mock_video_capture(total_frames, opened=True):
    ...
    retrieve_responses = [(True, np.zeros((10, 10, 3), dtype=np.uint8)) for _ in range(total_frames)]
...
def test_process_video_happy_path(pose_estimator, dummy_video_path):
    ...
    with mock.patch("cv2.VideoCapture", return_value=mock_cap):
        with mock.patch("cv2.cvtColor") as mock_cvt:
            mock_cvt.return_value = np.zeros((10, 10, 4), dtype=np.uint8)
            # ✅ Patch the exact location used in pose_estimator
            with mock.patch(
...
def test_process_video_frame_skip(pose_estimator, dummy_video_path):
    ...
    with mock.patch("cv2.VideoCapture", return_value=mock_cap):
        with mock.patch("cv2.cvtColor", return_value=np.zeros((10, 10, 4), dtype=np.uint8)):
            with mock.patch(
                "analysis.pose_estimator.mp_vision.PoseLandmarker.create_from_options"
            ) as mock_create:
                mock_create.return_value.__enter__.return_value = mock_landmarker
...
def test_process_video_caching(pose_estimator, dummy_video_path):
    ...
    with mock.patch("cv2.VideoCapture", return_value=second_cap) as mock_vc_ctor:
        with mock.patch("cv2.cvtColor") as mock_cvt:
            with mock.patch(
                "analysis.pose_estimator.mp_vision.PoseLandmarker.create_from_options"
            ) as mock_create:
