...
def test_get_landmark(make_landmarks):
    ...
    frame_data = FrameData(
        frame_number=0, timestamp_s=0.0, landmarks=landmarks, world_landmarks=landmarks
    )
    for i in range(33):
...
def test_get_world_landmark_by_name(make_landmarks):
    ...
    fd = FrameData(
        frame_number=0, timestamp_s=0.0, landmarks=landmarks, world_landmarks=world
    )
...
def test_get_visible_landmarks(make_landmarks):
    ...
    lms[10] = Landmark(
        x=2, y=2, z=2, visibility=0.71, presence=1.0
    )  # exactly above 0.7? property uses >0.7 so 0.7 excluded
...
def test_get_limb(make_landmarks):
    ...
    joints = get_landmark_indices_from_exercise("bicep_curl")[
        0
...
def test_frame_data_from_mediapipe(make_landmarks):
    ...
    world = [
        MockLandmark(i + 0.1, (i % 2) + 0.2, (i % 3) + 0.3, 0.9, 0.8) for i in range(33)
    ]  # mock world landmarks
