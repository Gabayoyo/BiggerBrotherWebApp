...
def make_limb_landmarks(
    shoulder_xyz, elbow_xyz, wrist_xyz, vis_shld=1.0, vis_elb=1.0, vis_wrs=1.0
):
    """
...
def build_frame_from_limbs(left_limb_landmarks, right_limb_landmarks=None):
    ...
    for idx, lm in zip(left_limb, left_limb_landmarks, strict=True):
        world[idx] = lm
    if right_limb_landmarks is not None:
        for idx, lm in zip(right_limb_indices, right_limb_landmarks, strict=True):
            world[idx] = lm
    ...
    return FrameData(
        frame_number=0, timestamp_s=0.0, landmarks=world, world_landmarks=world
    )


def build_exercise(
    name="bicep_curl", bilateral="bilateral", limbs=None, frame_data=None
):
...
def test_calculate_angle_division_by_zero():
    ...
    with pytest.raises(
        ValueError, match="Cannot compute angle: one of the vectors has zero length."
    ):
...
class TestBilateralAngle:
    ...
    def test_one_side_low_visibility_uses_other(self):
        ...
        frame = _make_bilateral_frame(
            ...
            (0, 0, 0),
            (0, 1, 0),
            0.6,  # 180° (straight arm? but irrelevant)
        )
        exercise = mock_exercise_bilateral([(11, 13, 15), (12, 14, 16)], [frame])
        angles = derive_angles(exercise)
        assert len(angles) == 1
